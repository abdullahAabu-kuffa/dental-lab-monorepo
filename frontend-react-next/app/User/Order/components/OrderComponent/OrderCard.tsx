...
export const OrderCard: React.FC<OrderCardProps> = ({
  ...
  return (
    <motion.div
      onClick={onClick}
      className={`
        flex items-center gap-4 rounded-2xl p-4 cursor-pointer transition-all duration-300
        ${getCardStyles()}
      `}
    ...
    >
      ...
      <div className="flex flex-col items-end gap-1">
        <span className={`
          text-xs font-bold rounded-lg px-3 py-1.5 border whitespace-nowrap
          ${statusColors.badge}
        `}>
          {getStatusDisplay(order.status)}
        </span>
        <span className={`text-xs font-semibold ${urgencyColors.text}`}>
          {order.urgency}
        </span>
      </div>
    </motion.div>
