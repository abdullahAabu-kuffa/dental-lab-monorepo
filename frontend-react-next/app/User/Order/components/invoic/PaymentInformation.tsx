...
export const PaymentStatus: React.FC<PaymentStatusProps> = ({ order, onPay }) => {
    ...
    <div className="bg-gray-50 rounded-2xl p-6  space-y-6 max-w-xl mx-auto">
      {isPaid ? (
        <div className="text-center space-y-3">
          ...
          <h2 className="text-green-500 text-xl font-bold">Payment Successful!</h2>
          <p className="text-gray-600">Your order has been placed and payment confirmed.</p>
          <p className="text-gray-400 text-sm">Transaction ID: {order.transactionId || "N/A"}</p>
        </div>
      ) : (
        <div className="text-center space-y-4">
          ...
          <div className="bg-gradient-to-br from-blue-50 to-indigo-100 rounded-2xl p-6 border border-blue-200">
            ...
            <div className="text-3xl font-bold text-blue-900 mb-2">
              {(order.totalAmount || 0).toLocaleString("en-US", { style: "currency", currency: "EGP" })}
            </div>
            <p className="text-blue-600 text-sm">Secure payment powered by our platform</p>
          </div>
      ...
      <div className="bg-white rounded-2xl p-6 shadow space-y-4">
        ...
        <div className="grid grid-cols-2 gap-y-2 gap-x-4 text-gray-700">
          ...
          <span>{(order.totalAmount || 0).toLocaleString("en-US", { style: "currency", currency: "EGP" })}</span>
          <span className="font-medium">Payment Method</span>
          <span>{order.paymentMethod || "-"}</span>
          <span className="font-medium">Date of Payment</span>
          <span>{order.paymentDate ? new Intl.DateTimeFormat("en-GB", { day: "2-digit", month: "short", year: "numeric" }).format(typeof order.paymentDate === 'string' ? new Date(order.paymentDate) : order.paymentDate) : "-"}</span>
