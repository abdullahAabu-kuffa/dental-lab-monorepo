            ...
            <div className="relative bg-white rounded-lg shadow-xl max-w-sm w-full z-10 border border-gray-200">
                ...
                <div className="px-6 pb-4 flex justify-end gap-3">
                    ...
                    <button
                        onClick={onConfirm}
                        disabled={isLoading}
                        className={`px-4 py-2 text-sm rounded-md ${isLoading ? 'bg-gray-400' : 'bg-green-600'} text-white hover:bg-green-700`}
                    >
                        {isLoading ? "Approving..." : "Approve"}  {/* Change text based on loading */}
                    </button>
                </div>
            </div>
        </div>
