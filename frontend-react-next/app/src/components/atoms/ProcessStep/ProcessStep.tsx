...
const ProcessStep = memo(function ProcessStep({ step, isLast = false }: ProcessStepProps) {
  ...
  const getStepStyles = () => {
    switch (step.status) {
      case 'completed':
      case 'active':
        return 'bg-[#C9A961] text-white shadow-lg';
      case 'pending':
  ...
  return (
    <div className="flex flex-col items-center relative">
      {/* Step Circle */}
      <div className={`w-16 h-16 rounded-full flex items-center justify-center transition-all duration-300 hover:scale-110 cursor-pointer ${getStepStyles()}`}>
        <IconComponent className="w-8 h-8" />
      </div>
      
      {/* Step Label */}
      <h3 className="mt-4 text-sm font-medium text-gray-800 text-center font-inter max-w-[120px]">
        {step.title}
      </h3>
    </div>
  );
