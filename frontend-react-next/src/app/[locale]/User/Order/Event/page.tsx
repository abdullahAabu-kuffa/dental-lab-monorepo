			...
			<div className="max-w-7xl mx-auto">
				...
				<div className="text-center mb-12">
					<h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4">
						{t("upcomingEventsAndScientificDays")}
					</h1>
				</div>
					...
					<div className="flex flex-col md:flex-row gap-4 items-center justify-between">
						...
						<div className="flex gap-4">
							<select className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
								...
								<option>{t("workshop")}</option>
								<option>{t("symposium")}</option>
								<option>{t("masterclass")}</option>
							</select>
							<select className="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
								<option>{t("status")}</option>
								<option>{t("upcoming")}</option>
								<option>{t("registered")}</option>
								<option>{t("eventEnded")}</option>
							</select>
						</div>
					</div>
				</div>
				...
				<div className="text-center">
					<div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
						<h2 className="text-2xl font-bold text-gray-900 mb-4">
							{t("wantToHostYourOwnEvent")}
						</h2>
						<p className="text-gray-600 mb-6">
							{t("partnerWithAvanteDentalLab")}
						</p>
						<button
