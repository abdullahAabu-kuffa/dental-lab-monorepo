    ...
    >
      <div
        className={`w-12 h-12 rounded-xl flex items-center justify-center transition-all duration-300 ${
          isSelected ? (isPaid ? "bg-green-100 text-green-600" : "bg-red-100 text-red-600") : "bg-slate-100 text-slate-600"
        }`}
      >
      ...
      <div className="flex flex-col items-end gap-1">
        <span className={`text-xs font-bold px-3 py-1.5 rounded-lg ${getBadgeStyles()}`}>
          {isPaid ? (
            <>
        ...
        <div className="whitespace-nowrap text-xs text-gray-500 mt-1">{new Date(invoices.createdAt).toLocaleDateString()}</div>
      </div>
    </motion.div>
  );
};
