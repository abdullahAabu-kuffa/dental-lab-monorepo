...
export const transformApiOrder = (apiOrder: ApiOrder): Order => {
  return {
    ...
    status: apiOrder.status === "PENDING" ? "Pending" : 
            apiOrder.status === "IN_PROGRESS" ? "In Progress" :
            apiOrder.status === "COMPLETED" ? "Completed" :
            apiOrder.status === "CANCELLED" ? "Cancelled" : "Pending",
    ...
    stages: {
      ...
      digitalDesign: { completed: apiOrder.status !== "PENDING", completedAt: apiOrder.status !== "PENDING" ? apiOrder.createdAt : "Pending" },
      manufacturing: { completed: apiOrder.status === "COMPLETED", completedAt: apiOrder.status === "COMPLETED" ? apiOrder.createdAt : "Pending" },
      qualityControl: { completed: apiOrder.status === "COMPLETED", completedAt: apiOrder.status === "COMPLETED" ? apiOrder.createdAt : "Pending" },
